...
test('should show an error on the dynamic form for a bad host', async ({
  ...
  // Trigger blur so the parameter validation request fires.
  ...
  // Submitting fires another validate-parameters call followed by the create
  // request, which is the one that ultimately surfaces the host error.
