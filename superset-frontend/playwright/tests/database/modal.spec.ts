...
/**
 * Each test starts on the database list page with the connect-database modal
 * open (matches the Cypress beforeEach flow).
 */
...
test('should open the dynamic database connection form with empty fields', async ({
  ...
  // Connection-detail fields should be empty before the user supplies any
  // credentials.
  ...
  // selected preferred-database tile (e.g. "PostgreSQL"), so we just assert
  // that it is rendered.
  await expect(
...
test('should show an error when the dynamic form has a bad host', async ({
  ...
  const validateOnBlur = waitForPost(page, VALIDATE_PARAMETERS_URL);
  ...
  const validateOnSubmit = waitForPost(page, VALIDATE_PARAMETERS_URL);
