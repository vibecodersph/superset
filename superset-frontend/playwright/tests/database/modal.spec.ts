...
/**
 * Names of the dynamic-form fields rendered by the connection wizard for
 * the first preferred database engine.
 */
...
const test = testWithAssets.extend<{ databaseModal: DatabaseModal }>({
  databaseModal: async ({ page }, use) => {
    const listPage = new DatabaseListPage(page);
    await listPage.goto();
    await listPage.waitForPageLoad();
    await listPage.clickCreateDatabase();
...
test('should open dynamic form', async ({ databaseModal }) => {
  await databaseModal.clickPreferredDatabase(0);

  // All dynamic-form fields start empty for a new connection
...
test('show error alerts on dynamic form for bad host', async ({
  ...
  // The Connect button auto-enables once the validate-parameters call resolves
