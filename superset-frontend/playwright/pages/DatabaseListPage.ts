 ...
 *
 * Provides primitives for navigating to the list and opening the
...
export class DatabaseListPage {
  ...
  /**
   * Locator for the "+ Database" create button.
   */
  getCreateDatabaseButton(): Locator {
    return this.page.locator(DatabaseListPage.SELECTORS.CREATE_BUTTON);
  }

  /**
   * Wait for the create-database button to be visible (page-load signal).
   */
  async waitForPageLoad(options?: { timeout?: number }): Promise<void> {
    await this.getCreateDatabaseButton().waitFor({
      state: 'visible',
      ...options,
    });
  }
